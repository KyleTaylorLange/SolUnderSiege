...
DECLARE_DELEGATE_RetVal_OneParam(FText, FOnGetPlayerStateAttribute, ALastimPlayerState*);
...
class LASTIM_API SScoreboardWidget : public SCompoundWidget
{
...
protected:
	...
	ALastimPlayerState* GetSortedPlayerState(const FTeamPlayer& TeamPlayer) const;
	...
	FText GetPlayerScore(ALastimPlayerState* PlayerState) const;

	/* Gets the player's score. */
	int32 GetAttributeValue_Score(ALastimPlayerState* PlayerState) const;

	/* Gets the player's kills. */
	int32 GetAttributeValue_Kills(ALastimPlayerState* PlayerState) const;

	/* Gets the player's deaths. */
	int32 GetAttributeValue_Deaths(ALastimPlayerState* PlayerState) const;

	/* Gets the player's score. */
	FText GetAttributeText_Score(ALastimPlayerState* PlayerState) const;

	/* Gets the player's kills. */
	FText GetAttributeText_Kills(ALastimPlayerState* PlayerState) const;

	/* Gets the player's deaths. */
	FText GetAttributeText_Deaths(ALastimPlayerState* PlayerState) const;
