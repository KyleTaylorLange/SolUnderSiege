...
#include "Bullet.h"
#include "LastimFirearm.generated.h"
...
struct FFireMode
{
	...
	TSubclassOf<class ALastimProjectile> ProjectileClass;
...
//UCLASS()
//class LASTIM_API ALastimFirearm : public ALastimWeapon
UCLASS(Abstract, config = Game)
class ALastimFirearm : public ALastimWeapon
{
	GENERATED_BODY()
	...
	/** Actually reloads weapon's ammo and removes clips. **/
	virtual class ALastimAmmo* ChooseBestAmmoItem();
...
public:
	...
	ALastimFirearm(const FObjectInitializer& ObjectInitializer);
	...
	virtual ALastimProjectile* GetModifiedProjectile(int32 FireModeNum);
...
public:
	...
	UPROPERTY(EditDefaultsOnly, Category = Firearm)
	TSubclassOf<class ALastimAmmo> DefaultAmmoClass;

	/* Current ammo/magazine loaded in this weapon. */
	UPROPERTY(Transient, Replicated)
	class ALastimAmmo* CurrentAmmoItem;

	/* Ammo/magazine we want to use on reload. */
	UPROPERTY(Transient, Replicated)
	class ALastimAmmo* PendingAmmoItem;

public:
	/* List of LastimAmmo items for this weapon; temporary until ammo is moved to the Character's inventory. */
	UPROPERTY()
	TArray<class ALastimAmmo*> AmmoInventory;
...
protected:
	...
	virtual void FireWeapon(float DamageScalar = 1.0f); //PURE_VIRTUAL(ALastimFirearm::FireWeapon, );
