...
ULastimCharacterMovementComponent::ULastimCharacterMovementComponent(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
	CrouchedHalfHeight = 56.25f;
	NavAgentProps.bCanCrouch = true;
	bCanWalkOffLedgesWhenCrouching = true;
	MaxSprintSpeed = 900.f; //750.f; // 900.f
	MaxWalkSpeed = 450.f; //375.f; //600.f; // 500.f;
...
float ULastimCharacterMovementComponent::GetMaxSpeed() const
{
	float MaxSpeed = Super::GetMaxSpeed();

	// TODO: Better handling if player is aiming and crouching.
	const ALastimCharacter* LastimCharacterOwner = Cast<ALastimCharacter>(PawnOwner);
	if (LastimCharacterOwner)
	{
		if (LastimCharacterOwner->IsAiming())
		{
			MaxSpeed *= MaxAimingSpeed / MaxWalkSpeed;
		}
		if (LastimCharacterOwner->IsSprinting())
		{
...
bool ULastimCharacterMovementComponent::DoJump(bool bReplayingMoves)
{
...
float ULastimCharacterMovementComponent::CalculateJumpVelocity() const
{
