...
void ATeamGameMode::HandleMatchIsWaitingToStart()
{
	...
	for (TActorIterator<APlayerStart> It(GetWorld()); It; ++It)
	{
		...
		ALastimPlayerStart* TeamPlayerStart = Cast<ALastimPlayerStart>(TestSpawn);
		if (TeamPlayerStart)
		{
...
void ATeamGameMode::InitBot(ALastimAIController* AIC, FBotProfile* InBotProfile)
{
...
ATeamState* ATeamGameMode::ChooseTeam(ALastimPlayerState* ForPlayerState) const
{
	ALastimGameState* const MyGameState = Cast<ALastimGameState>(GameState);
	...
	if (MyGameState)
	{
		...
		{
			ALastimPlayerState const* const TestPlayerState = Cast<ALastimPlayerState>(GameState->PlayerArray[i]);
...
bool ATeamGameMode::IsSpawnpointAllowed(APlayerStart* SpawnPoint, AController* Player) const
{
	...
	{
		ALastimPlayerStart* TeamStart = Cast<ALastimPlayerStart>(SpawnPoint);
		ALastimPlayerState* PlayerState = Cast<ALastimPlayerState>(Player->PlayerState);
...
float ATeamGameMode::ModifyDamage(float Damage, AActor* DamagedActor, struct FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	ALastimCharacter* DamagedPawn = Cast<ALastimCharacter>(DamagedActor);
	if (DamagedPawn && EventInstigator)
	{
		ALastimPlayerState* DamagedPS = Cast<ALastimPlayerState>(DamagedPawn->GetPlayerState());
		ALastimPlayerState* InstigatorPS = Cast<ALastimPlayerState>(EventInstigator->PlayerState);
...
bool ATeamGameMode::PlayersAreEnemies(ALastimPlayerState* AskingPlayer, ALastimPlayerState* TargetPlayer) const
{
