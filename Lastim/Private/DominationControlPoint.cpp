...
ADominationControlPoint::ADominationControlPoint(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	BaseMeshComp->RelativeLocation = FVector(0.f, 0.f, 0.f);
	...
	SymbolPSComp->RelativeLocation = FVector(0.f, 0.f, 100.f);
	...
	LightComp->RelativeLocation = FVector(0.f, 0.f, 100.f);
...
{
	ALastimCharacter* LChar = Cast<ALastimCharacter>(OtherActor);
	if (LChar)
	{
		ALastimPlayerState* PS = Cast<ALastimPlayerState>(LChar->GetPlayerState());
		if (PS)
		{
			OnTeamChange(PS);
		}
...
void ADominationControlPoint::OnTeamChange(ALastimPlayerState* NewOwner)
{
